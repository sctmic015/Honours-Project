...
class Controller:
    ...
    def joint_angles(self, t):
        ...
        # Sin and Cos waves scaled by 120 degrees
        sinewave = math.sin(t*2*math.pi/3)
        coswave = math.cos(t*2*math.pi/3)
        # Append the waves to the current input angles
        input_angles = np.append(self.current_angle, sinewave)
        input_angles = np.append(input_angles, coswave)
        # Get the current angles from the ANN
